...
$tpl->prepare();
if($_SESSION['atm_local_result']){
    if($_SESSION['atm_local_result']['code']){
        /*初始化payment物件*/
...
//        $returnXML = $card->parse_xmldata($_SESSION['atm_local_result']['content']);
        $returnXML = simplexml_load_string($_SESSION['atm_local_result']['content']);
//        /*更新訂單*/
//        $sql = $card->update_order($db,$returnXML);
        ...
        foreach($returnXML->Data->children() as $elm){
            ...
            if($elm->getName()=="RtnCode"){
                $idx = sprintf("%s",$elm);
                $tpl->assign("MSG_".strtoupper($elm->getName())."_STR",  Model_Order_Payment_Returncode_Allpay_Atm::$code[$idx]);
            }
        }
    }else{
        
    }
    unset($_SESSION['atm_local_result']);
}
$tpl->printToScreen();
